...
public async UniTask PlayCoronation(GameObject newPortrait, TextFader fader)
{
    ...
    // === ЭТАП 3: Показываем надпись "Да здравствует король!" ===
    if (textFader != null)
    {
        // Показываем текст
        textFader.ShowText("Да здравствует король!");
        
        // Ждём пока текст побудет на экране
        await UniTask.Delay((int)(textDisplayDelay * 1000));
        
        // Скрываем текст
        textFader.HideText();
        
        // Небольшая пауза после исчезновения текста
        await UniTask.Delay((int)(betweenPhasesDelay * 1000));
    }
    
    // === ЭТАП 4: Рамка едет в финальную позицию (только после текста!) ===
    if (frameToMove != null)
    {
        await frameToMove.DOAnchorPos(frameTargetPosition, frameMoveDuration)
            .SetEase(Ease.OutQuad)
            .AsyncWaitForCompletion(); // 🔥 Убрали SetDelay, так как задержка уже была выше
    }
